...
const App: React.FC = () => {
  ...
  // ---------------------------------------------------------------------------
  // --- LOGO SETTING (在此处替换LOGO) ---
  // 说明：请将下方的 URL 替换为您自己的图片链接（建议使用透明背景的PNG格式）。
  // 您可以上传到服务器，或者使用Base64编码，或者直接放入public文件夹引用。
  // ---------------------------------------------------------------------------
  const logoUrl = "https://placehold.co/400x150/000000/ffffff?text=KMCN+LOGO";
  // ---------------------------------------------------------------------------
  ...
  return (
    <div className={`min-h-screen bg-km-black text-gray-200 selection:bg-km-red selection:text-white`}>
      ...
      {/* Navigation / Header */}
      <nav className={`sticky top-0 z-40 backdrop-blur-md border-b border-white/10 bg-black/80`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            <div className="flex items-center gap-4">
              {/* Logo Section */}
              <div className="relative group">
                <div className="absolute -inset-1 bg-gradient-to-r from-km-red to-red-900 rounded-full blur opacity-25 group-hover:opacity-50 transition duration-200"></div>
                <img 
                  src={logoUrl}
                  onError={(e) => {
                  ...
                  alt="KMCN 深圳马伽术 Logo" 
                  className="relative h-12 w-auto object-contain rounded border border-white/10 shadow-2xl bg-black" 
                />
              </div>
            ...
            <div className="flex items-center gap-2 bg-neutral-900/80 rounded-full p-1 border border-white/10 shadow-inner overflow-x-auto max-w-[200px] sm:max-w-none scrollbar-hide">
               <button 
                ...
                >
                  ...
                  <span className="hidden sm:inline">军警战术</span>
