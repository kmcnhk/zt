...
const DetailView: React.FC<Props> = ({ theme, onClose, isMilitary }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm overflow-y-auto">
      <div className={`relative w-full max-w-5xl rounded-2xl shadow-2xl overflow-hidden bg-neutral-950 text-gray-200 border border-neutral-800`}>
        ...
        <div className="relative border-b border-neutral-800">
          ...
          <div className={`p-8 relative z-10`}>
            <button onClick={onClose} className="absolute top-6 right-6 p-2 rounded-full bg-black/20 hover:bg-white/10 transition text-gray-300 hover:text-white backdrop-blur">
              <X className="w-6 h-6" />
            </button>
                    ...
                    <tr key={i} className={`${rowHover} transition-colors`}>
                      ...
                      <td className="px-4 py-2 text-right">
                        <button 
                          ...
                          className={`text-xs px-2 py-1 rounded border border-neutral-700 text-gray-400 hover:bg-km-red hover:text-white hover:border-km-red transition-all`}
                        >
                          生成训练
                        </button>
                      </td>
